...
router.get("/device", checkAuth, async (req, res) => {
  try {
    ...
    //get balsass
    const balsas = await getBalsas(userId);
    ...
    devices.forEach((device, index) => {
      ...
      devices[index].balsa = balsas.filter(
        balsa => balsa._id == device.balsaId
      )[0];
...
router.post("/device", checkAuth, async (req, res) => {
  try {
    ...
    // console.log("************************************************\n\n\n")
    const rule = await createSaverRule(userId, newDevice.dId, true);
    // console.log("rule__________>>\n\n\n", rule)
    const device = await Device.create(newDevice);
    // console.log("device__________>>", device)
    // console.log("************************************************")
...
//get balsas
async function getBalsas(userId) {
  try {
