...
import Balsa from "../models/balsa.js";
...
router.post("/getdevicecredentials", async (req, res) => {
  try {
    ...
    var balsa = await Balsa.findOne({ _id: device.balsaId });
  ...
  try {
    ...
    if (lastNotif == 0) {
      ...
      if (lastNotifToNowMins > incomingAlarm.triggerTime) {
        console.log("TRIGGERED: ",incomingAlarm.sender.type);
        saveNotifToMongo(incomingAlarm);
        sendMqttNotif(incomingAlarm);
        if(incomingAlarm.sender.type == 'senderemail'){
          // sendNotifEmail(incomingAlarm);
        }
      }
    }
  } catch (error) {
...
function sendNotifEmail(incomingAlarm){
  console.log("incomingAlarm ",incomingAlarm);
  try {
    let transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.NOTIF_APP_EMAIL,
        pass: process.env.NOTIF_APP_EMAIL_PASS,
      },
    });
    const{variableFullName,deviceName,value,condition, payload}= incomingAlarm;
    const bodymsg = `<p style="color:black;">The variable:  <strong>${variableFullName}</strong> in device: <strong>${deviceName}</strong> is ${condition} ${value}; actual value: <strong>${payload.value}</strong></p>`
    let mailOptions = {
      from: process.env.NOTIF_APP_EMAIL,
      to: incomingAlarm.sender.description,
      subject: `Openspirulina Alarms notification:`,
      html: bodymsg,
    };
