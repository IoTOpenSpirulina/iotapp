...
router.post("/alarm-rule", checkAuth, async (req, res) => {
  try {
    ...
    console.log("newRule", newRule.sender)
...
// GET ALARMS
router.get("/alarm-rule", checkAuth, async (req, res) => {
  try {
    const userId = req.userData._id;
    // console.log("userId\n\n\n\n",userId)
    //get alarrule
    const alarmRule = await AlarmRule.find({ userId: userId });

    //mongoose array to js array
    const alarmRules = JSON.parse(JSON.stringify(alarmRule));
    ...
    console.log("ERROR GETTING ALARMS RULES");
    console.log(error);
  ...
  try {
    ...
    if (res.data.data && res.status === 200) {
      ...
      const payload_templ =
        ...
        ',"sender":{ "type":"' +
        newAlarm.sender.type +
        '","description":"' +
        newAlarm.sender.description +
        '"}}';
